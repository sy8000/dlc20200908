...
import cn.besbing.Service.ForgetPWD;
...
public class ForgetPWDImpl implements ForgetPWD {
    ...
    public Boolean validUsercode(String usercode){
        ...
        SmUser smUser = smuserService.selectUserPasswordByUserCode(usercode);
        if (smUser != null){
            isExist = true;
        }
        return isExist;
    ...
    public JSONObject confirmNewPassword(@RequestBody String newPassword){
        ...
        try{
            if (newPassword != null && newPassword.trim() != "" && codeArr[0].trim() != null && codeArr[0].trim() != ""){
                SmUser smUser = smuserService.selectUserPasswordByUserCode(codeArr[0]);
